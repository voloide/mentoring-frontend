              ...
              <span v-if="isEditable(props.row)">
                ...
                >
                  <template v-slot:append>
                    <q-icon name="close" @click="clearField('healthFacility')" class="cursor-pointer" />
                  </template>
                </q-input>
              </span>
            ...
            <q-td key="options" :props="props">
              <div class="col">
                ...
                <template v-else>
                  ...
                  <q-btn v-if="selectedHealthFacility.id === props.row.id" flat round class="q-ml-md" color="green-8" icon="done" @click="saveUpdate(props.row)">
                    <q-tooltip class="bg-green-5">Guardar Alteração</q-tooltip>
                  </q-btn>
                  <q-btn v-if="selectedHealthFacility.id === props.row.id" flat round class="q-ml-md" color="red-8" icon="close" @click="discardChanges">
                    <q-tooltip class="bg-green-5">Descartar Alteração</q-tooltip>
                  </q-btn>
                  <q-btn flat round class="q-ml-md" color="red-8" icon="delete" @click="deleteHealthFacility(props.row.id)">
                  </q-btn>
                </template>
              </div>
            </q-td>
...
onMounted(() => {
  ...
  healthFacilityService.getAll().then(results => {
    const res = healthFacilityService.piniaGetAll()
    loading.value = false
...
const districts = computed(() => {
  ...
  return data.value.province ? allDistricts.filter(d => d.province_id == data.value.province.id) : [];
});

const isEditable = (row) => row.id === null || selectedHealthFacility.value.id === row.id;
...
const editHealthFacility = (healthFacility) => {
  ...
  data.value = { ...healthFacility, province: healthFacility.district.province };
};

const saveUpdate = () => {
  const healthFacility = { id: selectedHealthFacility.value.id, ...data.value };
  healthFacility.district = data.value.district
  healthFacilityService.updateHealthFacility(useHealthFacility().createDTOFromHealthFacility(healthFacility)).then((res) => {
...
const deleteHealthFacility = (id) => {
  alertWarningAction('Tem certeza que deseja apagar a unidade sanitaria?').then((result) => {
    if (result) {
      healthFacilityService.deleteHealthFacility(id).then((response) => {
...
const addNewRow = () => {
  ...
  if (!newRowAdded.value) {
    ...
    searchResults.value.unshift({ id: null, healthFacility: '', province: '', district: '', acao: 'NOVA_LINHA' });
  } else {
