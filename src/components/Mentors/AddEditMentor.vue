                ...
                <div class="page-input-container q-pa-md">
                    <div class="q-mt-lg">
                        <div class="row items-center q-mb-md">
                            ...
                            <span class="q-pl-sm text-subtitle2">Identificação do Mentor</span>
                        </div>
...
const districts = computed(() => {
  if (
    ...
    return districtService.getAllDistrictByProvinceId(mentor.value.employee.locations[0].province.id);
  } else {
    return null;
  }
});
...
const filterPartners = (val, update, abort) => {
  ...
  if (val === '') {
    update(() => {
      filterRedPartners.value = stringOptions.value.map(
        (partner) => partner
      );
    });
...
const healthFacilities = computed(() => {
  if (
    ...
    return healthFacilityService.getAllOfDistrict(mentor.value.employee.locations[0].district.id);
  } else {
    return null;
  }
});
    ...
    update(() => {
      filterRedHealthFacilities.value = stringOptions.value
        ...
        .filter((healthFacility) => {
          return (
            healthFacility &&
            healthFacility.healthFacility.toLowerCase().indexOf(val.toLowerCase()) !== -1
          );
        });
    });
  }
